...
class TrafficSignal:
    def __init__(self,
        ...
        self.in_lanes = None
        # 车道长度
        self.in_lanes_length = None
        self.out_lanes = None
        self.out_lanes_length = None
    ...
    def initialize(self):
        ...
        self.in_lanes = list(dict.fromkeys(self.sumo.trafficlight.getControlledLanes(self.ts_id)))
        self.in_lanes_length = {lane_id: self.sumo.lane.getLength(lane_id) for lane_id in self.in_lanes}
        links = self.sumo.trafficlight.getControlledLinks(self.ts_id)
        out_lanes = set()
        ...
        self.out_lanes_length = {lane_id: self.sumo.lane.getLength(lane_id) for lane_id in self.out_lanes}
        self.observation_space = spaces.Box(
            low=0, high=1, shape=(len(self.in_lanes),), dtype=np.float32
        )
    ...
    def compute_state(self):
        ...
        for lane in self.in_lanes:
            ...
            density = min(1, vehicle_num / (self.in_lanes_length[lane] / vehicle_gap))  # 车道密度，最大为1
    ...
    def compute_reward_with_delay(self):
        current_time = self.sumo.simulation.getTime()
        if current_time < self.last_action_time + self.control_interval:
    ...
    def _compute_pressure(self):
        ...
        for link_group in links:
            for link in link_group:
                in_lane = link[0]
                out_lane = link[1]
                q_in = self.sumo.lane.getLastStepVehicleNumber(in_lane)
                q_out = self.sumo.lane.getLastStepVehicleNumber(out_lane)
                cap_in = self.in_lanes_length[in_lane] / 7.5
                cap_out = self.out_lanes_length[out_lane] / 7.5
                pressure += (q_in / cap_in) - (q_out / cap_out)
        return pressure
        # incoming = 0
        # outgoing = 0
        # links = self.sumo.trafficlight.getControlledLinks(self.ts_id)
