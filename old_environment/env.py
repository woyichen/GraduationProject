...
from old_environment.traffic_signal import TrafficSignal

if 'SUMO_HOME' in os.environ:
    tools = os.path.join(os.environ['SUMO_HOME'], 'tools')
    sys.path.append(tools)
else:
    sys.exit("Please declare the old_environment variable 'SUMO_HOME'")
...
class SumoEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for ts_id, ts in self.traffic_signals.items():
            states[ts_id] = ts.compute_state()
            rewards[ts_id] = ts.compute_reward_with_delay()
